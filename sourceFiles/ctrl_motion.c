...
void ctrl_motion_lpf1(float pos_ref_org, float *pos_ref_sh)
{
	float u[1] = { 0.0 };
	u[0] = pos_ref_org;
	math_output(Clpf1[0], xlpf1, Dlpf1[0], u, pos_ref_sh, 1, 1, 1);
	math_state(Alpf1[0], xlpf1, Blpf1[0], u, xlpf1, 1, 1);
	/*
...
void ctrl_motion_sh1(float u_org, float *u_sh)
{
	float u[1] = { 0.0 };
	u[0] = u_org;
	math_output(Csh1[0], xsh1, Dsh1[0], u, u_sh, 2, 1, 1);
	math_state(Ash1[0], xsh1, Bsh1[0], u, xsh1, 2, 1);
